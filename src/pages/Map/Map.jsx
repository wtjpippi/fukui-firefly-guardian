...
export default function MapPage() {
  ...
  const handleLocateUser = () => {
    ...
    // 追従開始
    map.closePopup(); // 無限ループ（autoPanとの競合）を防ぐ
    setIsLocating(true);
  ...
  const handleJumpToLocation = (locationId, lat, lng) => {
    if (map) {
      setShouldFollowUser(false); // ジャンプ時は追従解除
  ...
  const courseDetails = {
    '蛍観橋コース': {
      ...
      description: `じょんのび館裏手の砂防公園内にあり、整備された園地から定点で観賞するコースです。その名の通り蛍を観賞する橋、けいかんばしからは、ホタルの乱舞を上から見下ろすように観賞できます。
じょんのび館と併せて訪れる人も多く、小さいお子様も比較的観賞しやすいコースです。
盛期は6月中旬から下旬となります。`,
    },
    'ほたる遊歩道': {
      image: '/images/courses/yuhodo.jpg',
      description: `ほたるの里公園駐車場から最も近く、気軽にほたる観賞を楽しめるアクセス抜群の散策ルートです。
ルートに沿って流れる清流の心地よいせせらぎに耳を傾けながら、静かに散策を楽しむことができます。周囲は視界がひらけているため、天気の良い夜には満天の星空とほたるの光の競演を同時に見上げられるのもこのコースならではの贅沢なポイント。
さらに、このエリアでは「ゲンジボタル」と「ヘイケボタル」の両方が生息しており、それぞれの光り方や大きさの違いをじっくり観察することができます。`,
    },
    '堂ノ腰コース': {
      image: '/images/courses/donokoshi.jpg',
      description: `ほたるの里公園から、山間の林道を辿るコース。人工的な灯りが無いので幻想的なホタルの飛翔を観賞できます。
6月中旬最盛期のホタルによる光のトンネルはここでしかみられない絶景です。`,
    },
    '源平橋コース': {
      image: '/images/courses/genpei.jpg',
      description: `集落の西側を流れる矢垂川沿いのコースで、コースの由来となる平家橋、源氏橋のほか合わせて四つの木橋があります。
川のせせらぎと欄干から眺めるホタルの光は情緒に溢れ、他のコースにはない特別な趣きがあります。6月中旬が盛期となります。`,
    }
  };
      ...
      <div className="container">
        ...
        <div className="map-container">
          <button
            ...
            title={shouldFollowUser ? "追従を停止" : "現在地を表示"}
          >
            <Navigation
              size={20}
